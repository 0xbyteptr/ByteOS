...
static bool _device_write(const void* _buffer, uint64_t loc, size_t count)
{
  ...
  {
    fprintf(stderr,
            "error: Too many uninstall data entries! Please report "
            "this bug upstream.\n");
    return false;
  }
...
static int bios_install(int argc, char* argv[])
{
  ...
  if (uninstall_mode)
  {
    ...
    {
      fprintf(stderr,
              "error: Uninstall mode set but no "
  ...
  for (size_t i = 0; i < SIZEOF_ARRAY(lb_guesses); i++)
  {
    ...
    if (!strncmp(gpt_header.signature, "EFI PART", 8))
    {
      ...
      {
        fprintf(
            stderr, "Installing to GPT. Logical block size of %" PRIu64 " bytes.\n", lb_guesses[i]);
      }
      break;
    }
  }
  ...
  if (gpt)
  {
    ...
    {
      fprintf(
          stderr, "Secondary header at LBA 0x%" PRIx64 ".\n", ENDSWAP(gpt_header.alternate_lba));
    }
  ...
  if (gpt && gpt2mbr_allowed == true)
  {
    ...
    for (int64_t i = 0; i < (int64_t) ENDSWAP(gpt_header.number_of_partition_entries); i++)
    {
      ...
      if (gpt_entry.unique_partition_guid[0] == 0 && gpt_entry.unique_partition_guid[1] == 0)
      {
        continue;
      }
...
no_mbr_conv:;
  ...
  {
    fprintf(stderr,
            "error: Could not determine if the device has a valid "
            "partition table.\n");
  ...
  if (gpt)
  {
    ...
    if (part_ndx != NULL)
    {
      ...
      {
        fprintf(stderr,
                "error: Chosen partition for BIOS boot code is not of "
    ...
    else
    {
      ...
      fprintf(stderr,
              "error: Installing to a GPT device, but no BIOS boot "
  ...
  bios_boot_autodetected:
    ...
    {
      fprintf(stderr,
              "error: The partition selected to install the BIOS boot "
              "code to contains\n");

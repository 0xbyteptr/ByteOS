...
static bool store_uninstall_data(const char *filename) {
  ...
  for (size_t i = 0; i < uninstall_data_i; i++) {
    ...
    if (fwrite(uninstall_data[i].data, uninstall_data[i].count, 1, udfile) !=
        1) {
      goto fwrite_error;
    }
  }
...
static bool uninstall(bool quiet_arg) {
  ...
  for (size_t i = 0; i < uninstall_data_i; i++) {
    ...
    while (!_device_write(ud->data, ud->loc, ud->count)) {
      ...
      if (!quiet) {
        fprintf(
            stderr,
            "warning: Uninstall data index %zu failed to write, retrying...\n",
            i);
      }
      if (!device_flush_cache()) {
  ...
  if (print_cache_flush_fail) {
    fprintf(
        stderr,
        "error: Device cache flush failure. Uninstall may be incomplete.\n");
    ret = false;
  }
...
static int bios_install(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
      ...
      if (uninstall_file != NULL && !quiet) {
        fprintf(stderr,
                "warning: --uninstall-data-file already set. Overriding...\n");
      }
  ...
  for (size_t i = 0; i < SIZEOF_ARRAY(lb_guesses); i++) {
    ...
    if (!strncmp(gpt_header.signature, "EFI PART", 8)) {
      ...
      if (!quiet) {
        fprintf(stderr,
                "Installing to GPT. Logical block size of %" PRIu64 " bytes.\n",
                lb_guesses[i]);
      }
      break;
    }
  ...
  if (gpt) {
    ...
    device_read(&secondary_gpt_header,
  ...
  if (gpt && gpt2mbr_allowed == true) {
    ...
    for (int64_t i = 0;
         i < (int64_t)ENDSWAP(gpt_header.number_of_partition_entries); i++) {
      ...
      if (part_to_conv_i == 4) {
        if (!quiet) {
          fprintf(stderr,
                  "GPT contains more than 4 partitions, will not convert.\n");
        }
        goto no_mbr_conv;
      }
      ...
      if (part_to_conv[part_to_conv_i].lba_end -
              part_to_conv[part_to_conv_i].lba_start + 1 >
          UINT32_MAX) {
        if (!quiet) {
          fprintf(stderr,
    ...
    for (size_t i = 0; i < 33; i++) {
      device_write(empty_lba,
                   ((ENDSWAP(gpt_header.alternate_lba) - 32) + i) * lb_size,
                   lb_size);
    }
    ...
    for (size_t i = 0; i < part_to_conv_i; i++) {
      ...
      uint32_t sect_count =
          ENDSWAP((part_to_conv[i].lba_end - part_to_conv[i].lba_start) + 1);
...
no_mbr_conv:;
  ...
  if (gpt == 0) {
    ...
    part_too_low:
      fprintf(stderr,
              "error: A partition's start sector is less than 63, aborting.\n");
      goto cleanup;
    }
  ...
  if (gpt) {
    ...
    if (part_ndx != NULL) {
      ...
      if (!force &&
          memcmp("Hah!IdontNeedEFI", &gpt_entry.partition_type_guid, 16) != 0) {
        fprintf(stderr, "error: Chosen partition for BIOS boot code is not of "
      ...
      for (partition_num = 0;
           partition_num < ENDSWAP(gpt_header.number_of_partition_entries);
           partition_num++) {
        device_read(
            &gpt_entry,
            (ENDSWAP(gpt_header.partition_entry_lba) * lb_size) +
        ...
        if (memcmp("Hah!IdontNeedEFI", &gpt_entry.partition_type_guid, 16) ==
            0) {
          if (!quiet) {
            fprintf(stderr,
      ...
      fprintf(stderr, "error: Installing to a GPT device, but no BIOS boot "
  ...
  bios_boot_autodetected:
    ...
    if (!valid) {
      fprintf(stderr, "error: The partition selected to install the BIOS boot "
                      "code to contains\n");
...
static void enroll_config_usage(void) {
  printf(
      "usage: %s enroll-config <Limine executable> <BLAKE2B of config file>\n",
      program_name);
  printf("\n");
...
static void version_usage(void) {
  ...
  printf("\n");
  printf("    --version-only  Only print the version number without licensing "
         "info\n");
...
static void general_usage(void) {
  ...
  printf("\n");
  printf("    --print-datadir   Print the directory containing the bootloader "
         "files\n");
  printf("\n");

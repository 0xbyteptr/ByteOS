...
static void set_idt_entry(int n, void* handler, uint16_t sel, uint8_t flags, uint8_t ist)
{
...
void early_idt_init(void)
{
  ...
  for (int i = 0; i < 256; ++i)
  {
    set_idt_entry(i, __isr_panic, 0x08, 0x8E, 0); /* present, DPL=0, interrupt gate */
    if ((i & 63) == 0)
    {

...
int mia_get_width(MiaWindow *w) { return w ? w->w : 0; }

int mia_get_height(MiaWindow *w) { return w ? w->h : 0; }
...
void mia_paint_clipped(int max_h) {
  ...
  for (int z = 1; z <= max_z; ++z) {
    for (int i = 0; i < MAX_WINDOWS; ++i) {
      ...
      /* title bar */
      framebuffer_draw_rect(windows[i].x, windows[i].y, windows[i].w, 18,
                            0x101030);
      /* title text */
      psf_draw_text(windows[i].x + 6, windows[i].y + 2, windows[i].title,
                    0xFFFFFF);
      /* client area clipped */
      framebuffer_draw_rect(windows[i].x, windows[i].y + 18, windows[i].w,
                            draw_h > 18 ? draw_h - 18 : 0, windows[i].color_bg);
      /* border */
      framebuffer_draw_rect(windows[i].x, windows[i].y, windows[i].w, 1,
                            0x000000);
      framebuffer_draw_rect(windows[i].x, windows[i].y + draw_h - 1,
                            windows[i].w, 1, 0x000000);
      framebuffer_draw_rect(windows[i].x, windows[i].y, 1, draw_h, 0x000000);
      framebuffer_draw_rect(windows[i].x + windows[i].w - 1, windows[i].y, 1,
                            draw_h, 0x000000);
    }
  }
}

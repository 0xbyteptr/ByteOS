...
__attribute__((noreturn)) static void start_loaded_elf(void* arg)
{
  ...
  serial_puts("  entry = ");
  serial_puthex64((uint64_t) (uintptr_t) entry);
  serial_puts("\n");
...
{
  if (!path || !out_buf || !out_len || !is_embedded)
    return -1;
...
int spawn_elf_from_buf(void* buf, size_t len)
{
  ...
  for (int i = 0; i < eh->e_phnum; i++)
  {
    ...
    {
      memset(base + dest_off + ph[i].p_filesz, 0, ph[i].p_memsz - ph[i].p_filesz);
    }
  }
